...
const InitialComponents=({restaurants,filtered,restaurantsName})=>{
  ...
  return (
    <div className="row  Hello2 ">
      {restaurants.map((item, index) => (
        <div key={index} className=" col-3 card mb-3"  onClick={() => { Click(item.info.id) }}>
          <img src={`https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/${item?.info?.cloudinaryImageId}`} className="card-img-top restaurant-image" alt="..." />
          <div className="card-body">
            ...
            <div>
              ...
              <span style={{ margin: "10px" }}>{item?.info?.avgRating ? item.info.avgRating : item.info.avgRatingString}</span>
              <span>
              ...
              <span style={{ margin: "20px" }}>{item?.info?.sla?.slaString}</span>
            </div>
...
const Click = (id) => {
 ...
 return(
        <div className="col-10 d-flex flex-wrap justify-content-around Hello2 ">
          ...
          {restaurants.length==0 && filtered.length==0 ? <Shimmer/>:""}
{filtered.length>0 || restaurantsName !=""  ?  <FilteredRestaurants/>: <InitialRestaurants/>}
